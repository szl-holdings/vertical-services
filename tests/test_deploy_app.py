...
class VerticalServicesContractTests(unittest.TestCase):
    ...
    def test_root_and_control_plane(self):
        ...
        self.assertIn("Six engines. One governed runtime.", root.text)
    ...
    def test_every_engine_health_route(self):
    ...
    def test_stateful_routes_require_and_isolate_session_tokens(self):
        ...
        other = TestClient(self.module.app, headers={"X-SZL-Session": "other-session-token-01234567890123"})
    ...
    def test_lyte_ingest_summary_and_drift(self):
        for index in range(20):
            response = self.client.post(
                "/lyte/v1/metrics", json={"stream": "latency", "value": float(index)}
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_finance_rejects_non_positive_price_and_calculates(self):
        self.assertEqual(
            self.client.post("/finance/v1/observations", json={"symbol": "SZL", "price": 0}).status_code,
            422,
        )
        for price in (100, 103, 99, 106):
            self.assertEqual(
                self.client.post(
                    "/finance/v1/observations", json={"symbol": "SZL", "price": price}
                ).status_code,
                200,
            )
    ...
    def test_terra_and_counsel_workflows(self):
        listing = self.client.post(
            "/terra/v1/listings",
            json={"market": "Westchester", "price": 900000, "sqft": 3000, "noi_annual": 72000},
        )
        ...
        obligation = self.client.post(
            ...
            json={"clause": "8.2", "obligation": "Notify within five days", "severity": "high"},
        )
    ...
    def test_vessels_engine_is_live_but_public_surface_is_killinchu(self):
        health = self.client.get("/vessels/healthz").json()
        self.assertTrue(health["consolidated"])
        self.assertEqual(health["public_surface"], "SZLHOLDINGS/killinchu")
        for payload in (
            {"imo": "IMO1234567", "lat": 40.0, "lon": -70.0, "sog": 12.0, "ts": 1000},
            {"imo": "IMO1234567", "lat": 41.0, "lon": -69.0, "sog": 12.0, "ts": 5000},
        ):
            self.assertEqual(self.client.post("/vessels/v1/positions", json=payload).status_code, 200)
        risk = self.client.get("/vessels/v1/vessel/risk?imo=IMO1234567")
