...
class ReplayStoreMixin:
    ...
    def _invalidate_changed_payload(self, connection, vertical, session_scope, digest):
        """Latch a known mutation atomically, including change-then-restore writes."""
        ...
        for row in rows:
            ...
            # This write hook detects structural drift. Normal status reads also
            # evaluate real clock freshness; fixture/event timestamps are not a clock.
    ...
    def _dependency_changes(connection, vertical, session_scope, dependencies, now):
        ...
        for dep in dependencies:
            ...
            if withdrawn:
                ...
                try:
                    # Receipt IDs do not themselves bind summary bytes.
                    ...
                    summary_hash = hashlib.sha256(_json(summary).encode("utf-8")).hexdigest()
                    if summary_hash != dep["summary_sha256"]:
                        reason = "EVIDENCE_SUMMARY_CHANGED"
                    elif (row["observed_at"] != dep["observed_at"]
                          or row["expires_at"] != dep["expires_at"]):
                        reason = "EVIDENCE_RECORD_CHANGED"
    ...
    def register_assessment(self, *, vertical, session_scope, assessment_id,
                            kind, snapshot, now):
    ...
    def withdraw_payloads(self, *, vertical, session_scope, payload_digests, now):
        ...
        try:
            with self._lock, closing(self._connect()) as connection:
                connection.execute("BEGIN IMMEDIATE")
                # All-or-nothing and same response for absent or foreign records.
